...
- **批量模式**（`scripts/`，>5 份证书时用）三步流水线：
  1. `extract_ip_pdf.py` — pymupdf 批量提取 PDF 文本（扫描件自动标记）
  2. `parse_ip.py` — 正则解析受理/授权/软著/PCT 字段（竖排文本、跨行名单、日期归一）
  3. `fill_ip.py` — 审核后的记录写入目标表（自动检测已有行、绝不覆盖、序号顺延、月度统计一键重建）
